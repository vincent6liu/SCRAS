...
runMAST <- function(df, sg) {
  ...
  # we create the test matrix, so no additional filtering of cells is
  # added here
...
saveResult <- function(result, filename) {
  ...
  ordered <- resultDf[order(abs(resultDf$"hurdle.Pr(>Chisq)"), decreasing = TRUE), ]
  colnames(ordered)[9] = 'p'
  write.table(ordered, file = filename, row.names = TRUE, col.names = TRUE, sep = ",", quote = FALSE)
}
